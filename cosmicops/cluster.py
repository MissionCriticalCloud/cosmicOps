...
class CosmicCluster(CosmicObject):
    def get_all_hosts(self):
        return [CosmicHost(self._ops, host) for host in
                self._ops.cs.listHosts(fetch_list=True, clusterid=self['id'], listall='true').get('host', [])]

    def get_storage_pools(self):
        storage_pools = self._ops.cs.listStoragePools(fetch_list=True, clusterid=self['id'], listall='true').get(
            'storagepool', [])
